...
public class SelectMembersAdapter extends RecyclerView.Adapter<SelectMembersAdapter.MemberViewHolder> {
    ...
    public SelectMembersAdapter(Context context, List<User> contacts, HashMap<String, Boolean> selectedMembers) {
    ...
    public void onBindViewHolder(@NonNull MemberViewHolder holder, int position) {
        ...
        holder.checkBoxSelect.setChecked(selectedMembers.containsKey(user.getUserId()));
        
        // Set up checkbox listener
        holder.checkBoxSelect.setOnCheckedChangeListener((buttonView, isChecked) -> {
            if (isChecked) {
                // Add member with default status (not admin)
                selectedMembers.put(user.getUserId(), false);
            } else {
                // Remove member
                selectedMembers.remove(user.getUserId());
            }
        });
